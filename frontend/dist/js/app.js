...
function initApp() {
    ...
    if (detailContainer) {
        ...
        // 1. Thao tác định dạng văn bản (Bold, Italic, Underline...) và điều khiển Combobox cỡ chữ kiểu Word
        detailContainer.addEventListener('click', (e) => {
            // A. Xử lý click nút mũi tên của Combobox cỡ chữ
            const arrowBtn = e.target.closest('.combobox-arrow-btn');
            if (arrowBtn) {
                e.preventDefault();
            ...
            // B. Xử lý click chọn một cỡ chữ gợi ý từ dropdown của Combobox
            const comboboxItem = e.target.closest('.combobox-item');
            if (comboboxItem) {
                e.preventDefault();
                const val = comboboxItem.getAttribute('data-value');
            ...
            // C. Các nút định dạng thông thường (Bold, Italic...)
            ...
            // D. Ẩn dropdown khi click vào nơi bất kỳ khác trong detailContainer
            const dropdown = document.querySelector('.combobox-dropdown');
            if (dropdown && !e.target.closest('.font-size-combobox')) {
                dropdown.style.display = 'none';
            }
        });

        // Đóng dropdown cỡ chữ khi click ra ngoài hoàn toàn workspace
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.font-size-combobox')) {
                const dropdown = document.querySelector('.combobox-dropdown');
        ...
        const applyFontSize = (sizeVal) => {
            ...
            if (editor) {
                ...
                spanTags.forEach(tag => {
                    if (tag.style.fontSize === 'xx-large' || tag.style.fontSize === '-webkit-xxx-large' || tag.getAttribute('size') === '7') {
                        tag.removeAttribute('size');
                        tag.style.fontSize = sizeVal;
                    }
                });
        ...
        detailContainer.addEventListener('change', (e) => {
            const selectFontName = e.target.closest('.font-name-select');
            if (selectFontName) {
                document.execCommand('fontName', false, selectFontName.value);
                const editor = document.getElementById('preview-content-edited');
                if (editor) editor.focus();
                return;
            }

            const inputFontSize = e.target.closest('.font-size-input');
            if (inputFontSize) {
                let sizeVal = inputFontSize.value.trim();
                if (sizeVal) {
                    if (/^\d+$/.test(sizeVal)) {
                        sizeVal += 'px'; // Tự động thêm px nếu chỉ nhập số
                        inputFontSize.value = sizeVal;
                    }
        ...
        // 3. Thay đổi màu chữ và cỡ chữ tức thì khi nhập
        detailContainer.addEventListener('input', (e) => {
            ...
            if (inputFontSize) {
                let sizeVal = inputFontSize.value.trim();
                // Áp dụng ngay nếu người dùng gõ cỡ chữ hợp lệ (ví dụ: "13" hoặc "15px")
                if (/^\d+(px|pt|em|rem)?$/i.test(sizeVal)) {
                    if (/^\d+$/.test(sizeVal)) {
                        sizeVal += 'px';
                    }
                    applyFontSize(sizeVal);
